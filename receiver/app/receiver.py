...
class receiver_udp(DataReceiver):
    ...
    def login_info(self): 
       ...
       logging.basicConfig( 
        ...
        #filemode='a'  # 'a' para agregar, 'w' para sobrescribir cada vez
        )  
    ...
    async def inicializar_servidor(self):
        ...
        logging.info(f"Escuchando en {self.host}:514 (UDP)...")
            ...
            if data:
                try:
                    ...
                    if self._msg_counter % self._log_interval == 0:
                        logging.info(f"Recibidos {self._msg_counter} mensajes desde el arranque")
                    task =asyncio.create_task(self.process_syslog_message(decoded_message))
                    task.add_done_callback(self._on_task_processed)
                except Exception as e:
                    logging.error(f"Error procesando mensaje: {e}")
        except Exception as e:
            logging.error(f"Error en la conexión con {client_address}: {e}")
    
    def _on_task_processed(self, fut: asyncio.Future): #Funcion que cuenta cuantos mensajes se han procesado a redis
        # OJO: aquí ya terminó process_syslog_message
        self._processed_counter = getattr(self, "_processed_counter", 0) + 1
        if self._processed_counter % self._log_interval == 0:
            logging.info(f"Procesados {self._processed_counter} mensajes en tareas")
    ...
    async def process_syslog_message(self, message):
        ...
        try:
            ...
            #logging.info(f"Mensaje syslog procesado y enviado a Redis {json_str}")
            return True
        except Exception as e:
